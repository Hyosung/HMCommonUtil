...
#define kPRArrowNormal(FLAG) FLAG?@"icon_refresh_up@2x.png":@"icon_refresh_down@2x.png"
#define kPRArrowBlack(FLAG) FLAG?@"icon_refresh_up_black@2x.png":@"icon_refresh_down_black@2x.png"
#define kPRArrowWhite(FLAG) FLAG?@"icon_refresh_up_white@2x.png":@"icon_refresh_down_white@2x.png"

@interface LoadingView ()
- (void)updateRefreshDate :(NSDate *)date;
- (void)layouts;
- (NSString *)imagePathForDirection:(BOOL) flag;//YES:UP;NO:DOWN
- (UIColor *)dateColor;
@end
...
- (NSString *)imagePathForDirection:(BOOL)flag{
    NSString *path = @"";
    switch (_arrowStyle) {
        case kPRArrowStyleNormal:
            path = kPRArrowNormal(flag);
            break;
            
        case kPRArrowStyleBlack:
            path = kPRArrowBlack(flag);
            break;
            
        case kPRArrowStyleWhite:
            path = kPRArrowWhite(flag);
            break;
        default:
            break;
    }
...
- (UIColor *)dateColor{
    ...
    switch (_arrowStyle) {
        case kPRArrowStyleNormal:
            color = [[UIColor lightGrayColor] colorWithAlphaComponent:0.85];
            break;
            
        case kPRArrowStyleBlack:
            color = [[UIColor blackColor] colorWithAlphaComponent:0.85];
            break;
            
        case kPRArrowStyleWhite:
            color = [[UIColor whiteColor] colorWithAlphaComponent:0.85];
            break;
        default:
            break;
    }
...
- (void)setArrowStyle:(PRArrowStyle)arrowStyle{
...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
        ...
        UIFont *ft = [UIFont fontWithName:@"Arial Rounded MT Bold" size:12.5];
        _stateLabel = [[UILabel alloc] init];
        _stateLabel.font = ft;
        _stateLabel.textColor = [UIColor viewFlipsideBackgroundColor];
        _stateLabel.textAlignment = UITextAlignmentCenter;
        _stateLabel.backgroundColor = [UIColor clearColor];
        _stateLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        _stateLabel.text = NSLocalizedString(@"下拉以刷新...", @"");
        [self addSubview:_stateLabel];

        _dateLabel = [[UILabel alloc] init];
        _dateLabel.backgroundColor = [UIColor clearColor];
        _dateLabel.font = ft;
        _dateLabel.textColor = [self dateColor];
        _dateLabel.textAlignment = UITextAlignmentCenter;
        _dateLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        _dateLabel.text = NSLocalizedString(@"从未下拉过哦...", @"");
        ...
        _arrow.contents = (id)[UIImage imageNamed:[NSString stringWithFormat:@"PullingRefreshTableView.bundle/%@",[self imagePathForDirection:NO]]].CGImage;
...
- (void)layouts {
    ...
    if (self.isAtTop) {
        ...
        UIImage *arrow = [UIImage imageNamed:[NSString stringWithFormat:@"PullingRefreshTableView.bundle/%@",[self imagePathForDirection:NO]]];
        ...
        UIImage *arrow = [UIImage imageNamed:[NSString stringWithFormat:@"PullingRefreshTableView.bundle/%@",[self imagePathForDirection:YES]]];
        _arrow.contents = (id)arrow.CGImage;
        _stateLabel.text = NSLocalizedString(@"上拉以加载...", @"");
    }
...
- (void)setState:(PRState)state animated:(BOOL)animated{
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            _loading = YES;
            if (self.isAtTop) {
                _stateLabel.text = NSLocalizedString(@"奋力刷新中...", @"");
            } else {
                _stateLabel.text = NSLocalizedString(@"奋力加载中...", @"");
            }
            ...
            if (self.isAtTop) {
                _stateLabel.text = NSLocalizedString(@"可以松开啦...", @"");
            } else {
                _stateLabel.text = NSLocalizedString(@"松开加载更多...", @"");
            }
            
            if (!self.hidden) {
                [[SoundManager sharedManager] playSound:[Sound soundWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"PullingRefreshTableView.bundle/pull.wav" ofType:nil]]];
            }
...
- (void)setArrowStyle:(PRArrowStyle)arrowStyle{
    _arrowStyle = arrowStyle;
    _headerView.arrowStyle = arrowStyle;
    _footerView.arrowStyle = arrowStyle;
}
...
- (void)tableViewDidFinishedLoadingWithMessage:(NSString *)msg{
    ...
    [[SoundManager sharedManager] playSound:[Sound soundWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"PullingRefreshTableView.bundle/suc.wav" ofType:nil]]];
...
- (void)flashMessage:(NSString *)msg{
    //Show message
    __block CGRect rect = CGRectMake(self.bounds.size.width,self.window.bounds.size.height/3, self.bounds.size.width/2.0f, 36);
    ...
    if (_msgLabel == nil) {
        ...
        _msgLabel.layer.borderWidth = 1.5;
        _msgLabel.layer.borderColor = [[UIColor lightGrayColor] CGColor];
        _msgLabel.layer.cornerRadius = 6.0;
        _msgLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        _msgLabel.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.75];
        _msgLabel.textAlignment = UITextAlignmentCenter;
        [self.window addSubview:_msgLabel];
    }
    _msgLabel.text = msg;
